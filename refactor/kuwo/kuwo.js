...
function kw_cookie_get(url, callback) {
  kw_get_token((token) => {
    ...
    axios
      .get(url, {
        // todo 写死的 cookie 与 csrf，遇到问题的话，可以重新查看 listner1 的请求过程，再放置进来
        headers: {
          csrf: "LTSLFG5XFSG",
          "Cookie": "_ga=GA1.2.1886025126.1609055279; Hm_lvt_cdb524f42f0ce19b169a8071123a4797=1665812307; Hm_lpvt_cdb524f42f0ce19b169a8071123a4797=1665812307; kw_token=LTSLFG5XFSG",
...
function kw_render_tracks(url, page, callback) {
...
function kw_get_lrc(arr) {
  const lyric = arr.reduce((str, item) => {
    ...
    return `${str}[${num2str(m)}:${num2str(
      parseInt(s, 10)
    )}.${num2str(ms)}]${item.lineLyric}\n`;
  }, '');
  return lyric;
}
